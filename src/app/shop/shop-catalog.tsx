    ...
    <div className="mx-auto max-w-6xl px-5 py-10">
      <div className="mb-8 max-w-2xl">
        ...
        <p className="mt-2 text-[var(--muted)]">
          {unlimited
            ? `${currentPlanName ?? "슈퍼관리자"} 계정은 업그레이드가 필요 없습니다.`
            : currentPlanName
              ? `현재 ${currentPlanName} 플랜입니다. 아래에서 업그레이드할 수 있습니다.`
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {PAID_PLANS.map((id) => {
          ...
          const isCurrent = !unlimited && currentPlanId === id;
          return (
            <article
              key={id}
              className={`flex flex-col rounded-3xl bg-[var(--panel)] p-6 ring-1 ${
                isCurrent ? "ring-[var(--brand)]" : "ring-black/5"
              }`}
            >
              ...
              <button
                type="button"
                onClick={() => checkout(id)}
                disabled={loadingPlan === id || unlimited || isCurrent}
